...
export default function RMWorkflowView({ department, entries, onAddEntry, onUpdateEntry, scriptUrl }: Props) {
    ...
    const getData = (entry: Entry, name: string) => {
        ...
        // Special case for Remarks because there are two columns with same name in GS
        // We'll rely on the field order or just take the first match if searching by label
        // But the entry.data should have the keys from types.ts
        return entry.data[name] || entry.data[label] || '';
    };
                                ...
                                <tbody className="divide-y divide-zinc-100">
                                    {sortedEntries
                                        .filter(e => {
                                            ...
                                            if (activeTab === 'step2') return getData(e, 'planned1') && !getData(e, 'al2o3');
                                            if (activeTab === 'history') return getData(e, 'al2o3') || getData(e, 'remarks_chemical');
                                            return true;
                                        })
                                        .length === 0 ? (
